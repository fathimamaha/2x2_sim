...
submit_array_job() {
    ...
    pushd ../run-edep-sim
        sbatch --array=[${array_values_string}] -p knl-preemptable  -o $logdir/output/rock/slurm-%j_out.txt --error=$logdir/error/rock/slurm-%j_error.txt --account=LCRC_for_DUNE ./runs/MicroRun3.rock.edep-sim.sh
    popd
}
