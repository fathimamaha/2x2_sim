...
submit_array_job() {
    ...
    pushd ../run-edep-sim
        sbatch --array=[${array_values_string}] -o $logdir/output/nu/slurm-%j_out.txt --error=$logdir/error/nu/slurm-%j_error.txt -N 4 --account=LCRC_for_DUNE ./runs/MicroRun3.nu.edep-sim.sh
    popd
}
